...
from src.blm._legacy.three_decisions import ThreeDecisionsEngine, generate_blm_strategy
...
def generate_ppt(target, competitors, style, output_dir, quarters, canva_export):
    ...
    """
    try:
        from src.blm._legacy.ppt_generator import BLMPPTGenerator, PPTX_AVAILABLE
    except ImportError:
...
def germany_analysis(output_dir, report_format, style):
    ...
    from src.blm._legacy.germany_telecom_analysis import GermanyTelecomBLMAnalyzer
    ...
    if report_format in ("ppt", "all"):
        try:
            from src.blm._legacy.ppt_generator import BLMPPTGenerator
...
def germany_analysis_enhanced(output_dir, style):
    ...
    try:
        from src.blm._legacy.ppt_generator_enhanced import BLMPPTGeneratorEnhanced
...
def comprehensive_analysis(output_dir):
    ...
    try:
        from src.blm._legacy.comprehensive_analysis_ppt import generate_comprehensive_ppt
    except ImportError as e:
